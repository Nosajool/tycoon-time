...
%Plan for Version 3.4----Read names of workshop toys?/whatever product and purchase them to sell in store. Set quantity etc...
...
var file : int
var numRecipes : int := 4 %Change this whenever you add a new recipe
...
procedure Recipes
    ...
    for i : 1 .. numRecipes %@
	var numMatType, matType, numMat : int
	get : file, numMatType

	for j : 1 .. numMatType
	    get : file, matType
	    get : file, numMat
	    if numGems (matType) >= numMat then
		workShop := true
	    else
		workShop := false
	    end if
	    ...
	    Font.Draw (intstr (numMat), 50 * matType, 600 - 100 * i, font2, black)
	end for
