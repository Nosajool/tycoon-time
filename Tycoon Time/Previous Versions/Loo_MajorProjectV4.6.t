...
%Plan for Version 5.0----Work on Store Timers
...
procedure StoreTimers
    for i : 1 .. 5
	if endTime - startStoreTimer (i) > sitem (i).timer then
	    sitem (i).quantity -= 1
	    money += sitem (i).sell
	end if
    end for
...
procedure ViewItemsOnSale
    ...
    for i : 1 .. 5
	x1 := -100 + i * amount
	Font.Draw (sitem (i).name, x1, y1, font1, black)
	Pic.ScreenLoad ("pics/items/" + sitem (i).name + ".bmp", x1 + 50, y1 - 50, picMerge)
	Font.Draw ("Being Sold at:", x1, y1 - 70, font3, black)
	Font.Draw (realstr (sitem (i).sell, 0) + "$ each", x1 + 30, y1 - 90, font3, black)
	Font.Draw ("Sell Time:", x1, y1 - 110, font3, black)
	Font.Draw (realstr (sitem (i).timer, 0) + "s each", x1 + 30, y1 - 130, font3, black)
	Font.Draw ("# Left to Sell:", x1, y1 - 150, font3, black)
	Font.Draw (intstr (sitem (i).quantity), x1 + 60, y1 - 170, font3, black)
	Font.Draw ("Total Time", x1, y1 - 210, font3, black)
	Font.Draw ("Remaining:", x1, y1 - 230, font3, black)
	Font.Draw (realstr (sitem (i).timer * sitem (i).quantity, 0) + "s", x1 + 30, y1 - 250, font3, black)
	Font.Draw ("Total Price:", x1, y1 - 290, font3, black)
	Font.Draw (realstr (sitem (i).sell * sitem (i).quantity, 0) + "$", x1 + 30, y1 - 310, font3, black)
    end for
