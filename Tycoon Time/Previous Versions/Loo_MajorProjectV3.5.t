...
%Plan for Version 3.5-----Read names of workshop toys?/whatever product and purchase them to sell in store. Set quantity etc...
...
%Recipes Materials
var FileInput : string := "recipes.txt"
var file : int
...
procedure DrawUpgradeBoxes
	...
	for j : 1 .. 10
		...
		if factoryUpgrade (i, j) = true then
		    ...
		    Pic.ScreenLoad ("pics/gems/" + intstr (i) + intstr (j) + ".bmp", boxx - 40, boxy, picMerge)
		else
		    drawfillbox (boxx, boxy, boxx + boxh, boxy + boxh, black)
		end if
	    end if
...
procedure Recipes
    ...
    for i : 1 .. numRecipes %@
	var numMatType, matType, numMat, x1, x2, y1, y2, col, counter : int
	counter := 0
	var item : string
	get : file, numMatType
	get : file, item
	...
	for j : 1 .. numMatType
	    ...
	    if numGems (matType) >= numMat * quantity (numRecipes) then
		workShop (j) := true
	    else
		workShop (j) := false
	    end if
	    ...
	    Pic.ScreenLoad ("pics/gems/" + intstr (matType) + "1.bmp", 50 * matType, 630 - 100 * i, picMerge)
	    %Number of Gems
	    Font.Draw ("x" + intstr (numMat), 50 * matType, 600 - 100 * i, font2, black)
	end for
